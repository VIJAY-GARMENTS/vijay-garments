...
<th
    {{ $attributes->merge(['class' => 'px-2 py-2 bg-zinc-50 border border-gray-300'])->only('class') }}
>
    @unless ($sortable)
        <button {{ $attributes->except('class') }} class="w-full
        ...
        uppercase tracking-wider group focus:outline-none focus:underline">
            <span class="w-full text-{{$align}}">{{ $slot }}</span>
        </button>
    @else
        <button {{ $attributes->except('class') }} class="w-full
        flex items-center space-x-1 px-3
