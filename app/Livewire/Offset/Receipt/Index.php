...
class Index extends Component
{
    ...
    {
        $this->redirect(route('receiptsoffset.upsert', ['0']));
    }


    public function getList()
    {
        return Receipt_offset::search($this->searches)
    ...
    {
        $obj=$this->getObj($id);
        DB::table('receiptitem_offsets')->where('receipt_offset_id', '=', $this->vid)->delete();
        $obj->delete();
    ...
    {
        return view('livewire.offset.receipt.index')->with([
            'list' => $this->getList()
        ]);
    }
}
